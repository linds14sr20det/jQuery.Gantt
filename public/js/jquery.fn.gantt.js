            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if ( i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage ) ){
                        ...
                        var className = entry.values[0].label.split("-").slice(0,2).join(" ");

                        if (name != className) {
                            entries.push('<div class="row name row' + k + (entry.desc ? '' : ' fn-wide') + '" id="rowheader' + k + '" offset="' + k % settings.itemsPerPage * tools.getCellSize() + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + (entry.name || '') + '</span>');
                            entries.push('</div>');

                            if (entry.desc) {
                                entries.push('<div class="row desc row' + k + ' " id="RowdId_' + k + '" data-id="' + entry.id + '">');
                        ...
                        if( entry.values[0].label.indexOf('Maint') > -1) {
                            name = className;
                        }
                    }
                });
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
                                    ...
                                    // find row
                                    var name = day.label.split("-").slice(0,2).join(" ");
                                    if (name == maintIssue && name.indexOf('Maint') > -1) {
                                        var topEl = $(element).find("#rowheader" + k);
                                    } else {
                                        maintIssue = name;
                                        k++;
                                        var topEl = $(element).find("#rowheader" + k);
